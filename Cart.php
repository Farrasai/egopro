...
class Cart
{
    ...
    public function addCartItem($item, $keepDiscount = false, $keepTax = false, $dispatchEvent = true)
    {
        ...
        if ($content->has($item->rowId)) {
            ...
            // $item->tanggalSewa += $content->get($item->rowId)->tanggalSewa;
            // $item->tanggalPengembalian += $content->get($item->rowId)->tanggalPengembalian;
            // $item->lamaSewa += $content->get($item->rowId)->lamaSewa;
        }
